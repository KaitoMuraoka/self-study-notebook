...
ここで、$\ket{\tilde{\lambda}}$は測定さえれたとき$\lambda$の良い測定量を与える状態を表す。

\subsection{$\lambda$が$n$桁の2進数で表せられない場合}
$\lambda$が2進数で表せない場合について、位相推定アルゴリズムがどのような状態を返すのか考える。

式(\ref{eq: Lion})より、係数を
\begin{eqnarray}
    a(k) = e^{2\pi i j k}
\end{eqnarray}
とすると、
$a(k)$は、$k$に対して$j$という振動数を持って振動していることがわかる。そのため、フーリエ変換後には$j$に対応する$\ket{j}$の振幅だけが残る。フーリエ変換の性質から、 $\lambda$がちょうど$n$桁の2進数で表せなくても、近似値にあたる$\ket{\tilde{j}}$周辺の振幅が大きくなる。

では、高い確率で$\lambda$を推定するにはどうすれば良いか考える。$b$を$0$から$2^n - 1$の範囲の整数として、$b / 2^t = 0.b_1 b_2 \cdots b_n$が、$j$より小さく$j$の最良の$n$ビット近似になるように選ぶとする。つまり、$j$と$b/2^n$の差$\delta \equiv j - b/2^n$は、$0 \leq \delta \leq 2^{-t} $を満たす。
